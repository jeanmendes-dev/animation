...
# ============================================================
# SCRIPT 01 - BDS / PARAMCD-based extraction
# Output: galaxi3_bds_features.csv + galaxi3_bds_coverage_report.csv
# ============================================================

BDS_DATASETS <- c("adbdc", "adcdai", "adice", "adlbef", "adrxfail", "adsaf", "advscdai")
PARAM_LEVEL_VARS <- c("PARAM", "AVAL", "AVALC", "BASE", "CHG", "PCHG", "DTYPE", "ABLFL", "APOBLFL",
                      "ANL01FL", "ANL02FL", "ANL03FL", "ANL04FL", "ANL05FL", "ANL06FL", "ANL07FL",
                      "AVALCAT1", "AVALCAT2", "PARCAT1", "PARCAT2", "CRIT1", "CRIT1FL", "CRIT2", "CRIT2FL",
                      "SRCDOM", "SRCVAR", "SRCSEQ")

extract_bds_one <- function(dataset_name) {
  ...
  if (!all(c("USUBJID", "AVISIT", "AVISITN", "PARAMCD") %in% names(df))) {
    return(list(data = NULL, coverage = make_coverage_row(dataset_name, df, NULL, character(), "not PARAMCD-based or missing required keys")))
  }

  id_cols <- c("USUBJID", "AVISIT", "AVISITN", "PARAMCD")
  value_vars <- intersect(PARAM_LEVEL_VARS, setdiff(names(df), id_cols))

  long <- df %>%
    select(all_of(id_cols), any_of(value_vars)) %>%
    pivot_longer(cols = all_of(value_vars), names_to = "source_var", values_to = "value",
                 values_transform = list(value = as.character)) %>%
    mutate(feature = paste0("AVAL_", toupper(dataset_name), "_", safe_name(PARAMCD), "_", safe_name(source_var))) %>%
    group_by(USUBJID, AVISIT, AVISITN, feature) %>%
    summarise(value = collapse_distinct(value), .groups = "drop")

  wide <- long %>% pivot_wider(names_from = feature, values_from = value) %>% arrange(USUBJID, AVISITN, AVISIT)
  out_cols <- setdiff(names(wide), CONFIG$key_vars)
  notes <- paste0("PARAMCDs=", n_distinct(df$PARAMCD), "; PARAM-level source vars=", length(value_vars), "; pivoted wider by PARAMCD")
  list(data = wide, coverage = make_coverage_row(dataset_name, df, wide, out_cols, notes))
}

run_bds_extraction <- function() {
  log_info("Starting BDS/PARAMCD extraction")
  res <- lapply(BDS_DATASETS, extract_bds_one); names(res) <- BDS_DATASETS
  features <- bind_rows(lapply(res, `[[`, "data")) %>% distinct()
  coverage <- bind_rows(lapply(res, `[[`, "coverage"))
  fwrite(features, file.path(CONFIG$output_dir, "galaxi3_bds_features.csv"))
  fwrite(coverage, file.path(CONFIG$output_dir, "galaxi3_bds_coverage_report.csv"))
  print(coverage)
