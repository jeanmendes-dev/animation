...
pivot_wide_by_seq <- function(df, dataset_name) {
  ...
  if (length(vary_cols) > 0) {
    df_long <- df %>%
      select(USUBJID, event_num, all_of(vary_cols)) %>%
      pivot_longer(cols = all_of(vary_cols),
                   names_to  = "var_name",
                   values_to = "value") %>%
      mutate(col_name = paste0(
        ifelse(is.numeric(value), "AVAL_", "AVAL_"),
        DS, "_", var_name, "_", event_num
      ))

    # Pivotar para wide: 1 coluna por variável × evento
    df_wide_vary <- df_long %>%
